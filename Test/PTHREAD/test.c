...
int main(void)
{
	...
	printf("main thread tid [0x%x] join thread 1, return number %d",
						(unsigned int)pthread_self(), *(int *)reval);
	pthread_create(&tid, NULL ,thread_2, NULL);
	pthread_join(tid, &reval);
	printf("main thread tid [0x%x] join thread 1,exit code  %d",
						(unsigned int)pthread_self(), *(int *)reval);
	...
	pthread_cancel(tid);
	pthread_join(tid, &reval);
	printf("main thread tid [0x%x] join thread 1,cancel code  %d",
						(unsigned int)pthread_self(), *(int *)reval);
